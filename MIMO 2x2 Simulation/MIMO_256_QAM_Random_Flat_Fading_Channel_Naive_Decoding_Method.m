...
M = 256;  % Modulation order (64-QAM)
k = log2(M); % Number of bits per symbol
Ts = 1e-6; % symbol period in seconds, adjust based on your requirements

% Initialize
H = randn(2,2) + 1j*randn(2,2); % MIMO channel
x = randi([0 1],N*k,1); % Information bits

modSignal = qammod(x,M,'InputType','bit','UnitAveragePower',true); % 64-QAM modulation
...
receivedBits = qamdemod(receivedSignal,M,'OutputType','bit','UnitAveragePower',true);
...
% EVM Calculation
refSignal = qammod(x,M,'InputType','bit','UnitAveragePower',true); % Reference signal
